...
from transformers import vm_info_to_markdown, servers_list_to_markdown, status_to_markdown, disks_list_to_markdown
...
def vm_ip(proxmox, node_name: str, vm_id: str) -> str:
    logging.info(f"Fetching current external IP address for VM '{vm_id}' on node '{node_name}'")
    agent_network_info = proxmox.nodes(node_name).qemu(vm_id).agent('network-get-interfaces').get()
    
    ip_addresses = []
    for iface in agent_network_info.get('result', []):
        for ip_info in iface.get('ip-addresses', []):
            ip_address = ip_info.get('ip-address')
            if ip_info.get('ip-address-type') == 'ipv4' and ip_address not in ['127.0.0.1']:
...
def vm_list_disks(proxmox, node_name: str, vm_id: str) -> str:
    ...
    for key, value in vm_config.items():
        if key.startswith(('virtio', 'scsi', 'ide', 'sata')) and not key.startswith('scsihw'):
            # Extract disk size and possibly other details if present
            disk_details = value.split(',')
            disk_size = next((detail.split('=')[1] for detail in disk_details if detail.startswith('size')), 'Unknown size')
...
def vm_resize_disk(proxmox, node_name: str, vm_id: str, disk_id: str, new_size: str) -> str:
    logging.info(f"Attempting to resize disk '{disk_id}' for VM '{vm_id}' on node '{node_name}' to {new_size}")

    # Validate new_size format (e.g., '30G' or '1024M')
    if not re.match(r'^\d+(G|M)$', new_size):
        return ("Invalid size format. Please specify the new size in gigabytes or megabytes, "
                "e.g., '30G' for 30 GB or '1024M' for 1024 MB.")

    resize_result = proxmox.nodes(node_name).qemu(vm_id).resize.put(disk=disk_id, size=new_size)
...
def vm_stats(proxmox, node_name: str, vm_id: str) -> str:
    ...
    cpu_usage_percentage = f"{cpu_usage * 100:.2f}%" if cpu_usage != "Unknown" else "Unknown"

    # Convert memory usage to GB and round to two decimal places
    mem_usage_gb = f"{bytes_to_gb(mem_usage):.2f} GB" if mem_usage != "Unknown" else "Unknown"
    max_mem_gb = f"{bytes_to_gb(max_mem):.2f} GB" if max_mem != "Unknown" else "Unknown"
...
def vm_adjust_resources(proxmox, node_name: str, vm_id: str, cpu_cores: str, memory_mb: str) -> str:
    proxmox.nodes(node_name).qemu(vm_id).config.put(cores=cpu_cores, memory=int(memory_mb))
    return f"Resources adjusted for VM {vm_id}: {cpu_cores} CPU cores, {memory_mb}MB RAM."

@command("Initiates a backup for a specified VM")
def vm_backup(proxmox, node_name: str, vm_id: str, storage: str, backup_id: str) -> str:
    logging.info(f"Initiating backup for VM ID: {vm_id} on storage {storage} with backup ID {backup_id}")
    backup_job_id = proxmox.nodes(node_name).vzdump.create(vmids=vm_id, storage=storage, mode='snapshot', compress='gzip', backup_id=backup_id)
    return f"Backup initiated for VM {vm_id} on storage {storage}. Job ID: {backup_job_id}"

@command("Restores a VM from a specified backup")
def vm_restore(proxmox, node_name: str, backup_file: str, storage: str) -> str:
    logging.info(f"Restoring VM from backup file {backup_file} on storage {storage}")
    proxmox.nodes(node_name).storage(storage).content.post(volid=backup_file, restore=1)
...
def vm_list_backups(proxmox, node_name: str, storage: str, vm_id: str = None) -> str:
    logging.info(f"Listing backups for storage: {storage} on node: {node_name} for VM ID: {vm_id if vm_id else 'All VMs'}")
    backup_files = proxmox.nodes(node_name).storage(storage).content.get()
    backups_list = []
    for backup in backup_files:
        if backup['content'] == 'vzdump' and (vm_id is None or (vm_id in backup['volid'])):
            backups_list.append({
                "volume": backup['volid'],
    ...
    if backups_list:
        ...
        return f"No backups found for VM ID: {vm_id}" if vm_id else "No backups found in storage: " + storage