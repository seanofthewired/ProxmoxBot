...
def resolve_vm_identifier(node_name: str, identifier: Union[str, int]) -> Optional[Union[int, str]]:
    ...
    logging.info(f"Resolving VM identifier. Node: {node_name}, Identifier: {identifier}")
    ...
    try:
        ...
        for vm in vms:
            ...
            if isinstance(identifier, int):
                logging.info(f"Matching identifier as int: {vm['vmid']} == {identifier}")
                if vm['vmid'] == identifier:
                    return vm['vmid']
            elif isinstance(identifier, str):
                logging.info(f"Matching identifier as str: {vm['name']} == {identifier} or {vm['vmid']} == {identifier}")
